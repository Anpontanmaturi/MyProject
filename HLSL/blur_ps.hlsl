...
cbuffer PARAMETARS_CONSTNT_BUFFER : register(b2)
{
...
float4 main(VS_OUT pin) : SV_TARGET
{
    ...
    const int gaussian_harf_kernel_size = 3;
    //const float gaussian_sigma = 1.0f;
    [unroll]
    for (int x = -gaussian_harf_kernel_size; x <= +gaussian_harf_kernel_size; x += 1)
    {
        [unroll]
        for (int y = -gaussian_harf_kernel_size; y <= +gaussian_harf_kernel_size; y += 1)
        {
