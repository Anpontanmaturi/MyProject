...
public:
	...
	int run()
	{
		...
		ImGui_ImplDX11_Init(graphics_system->GetDevice(), graphics_system->GetContext());
...
#if 1
		BOOL fullscreen = 0;
		graphics_system->GetSwapChain()->GetFullscreenState(&fullscreen, 0);
		if (fullscreen)
		{
			graphics_system->GetSwapChain()->SetFullscreenState(FALSE, 0);
		}
#endif
