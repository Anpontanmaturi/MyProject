...
StaticMesh::StaticMesh(ID3D11Device* device, const wchar_t* obj_filename, bool invert_uv)
{
	...
	{
		iterator->index_count = (iterator - 1)->index_count - iterator->index_start;
	}
	...
	while (fin)
	{
		...
		// 16
		else if (0 == wcscmp(command, L"mao_bump") || 0 == wcscmp(command, L"bump"))
		{
			fin.ignore();
	...
	create_vs_from_cso(device, "Shader\\static_mesh_vs.cso", vertex_shader.GetAddressOf(),
	...
	{
		/*load_texture_from_file(device, material.texture_filename.c_str(),
		material.shader_resource_view.GetAddressOf(), &texture2d_desc);*/
