...
void FreeCameraController::sync_camera_to_controller(const Camera& camera)
{
	eye = camera.get_eye();
	...
	// 回転角度を算出
	const DirectX::XMFLOAT3& front = camera.get_front();
...
void FreeCameraController::sync_controller_to_camera(Camera& camera)
{
	camera.set_lookat(eye, focus, up);
}
